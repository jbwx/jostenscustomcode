...
function JostensFunc() {
  ...
  if (planid) {
    //const URLinterval = setInterval(checkPage, 100); // not used. Ask about this, Jake
    let x = document.querySelector('[title="Add School Contact"]');
    if (x) {
...
function PasteBtnFunc() {
  ...
  if (!x) { // If the element doesn't exist, execute the following block
    ...
    document.getElementById("pastebtn1").addEventListener("click", myClick2); // Add click event listener to the new button
    document.getElementById("pastespan1").addEventListener("mouseover", myHover2); // Add mouseover event listener to the first span
  }
}

function myClick2() { // This seems a bit unnecessary. Come back to this later, Jake
  pasterFunc();
}
...
function clearclickFunc() { // come back to this later, Jake
...
function CopyBtnFunc() {
  ...
  if (!x) { // If the element doesn't exist, execute the following block
    ...
    btn.addEventListener("click", myClick3); // Add click event listener to the new button
